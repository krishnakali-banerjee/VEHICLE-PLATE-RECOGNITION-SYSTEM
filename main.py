...
platecascade = cv2.CascadeClassifier("C:\\Computer_Vision\\License_plate_recognition\\haarcascade_russian_plate_number.xml")
minArea = 500
...
while True:
    ...
    for (x, y, w, h) in numberplates:
        ...
        if area > minArea:
            ...
            cv2.putText(frame, "Number Plate Detected", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
            ...
            (thresh, frameReg) = cv2.threshold(plate_gray, 127, 255, cv2.THRESH_BINARY)
    ...
    if key == ord('s'):
        ...
        cv2.putText(frame, "SCAN SAVED", (15, 265), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 255, 255), 2)
        cv2.imshow("Result", frame)
        cv2.waitKey(500)
        count += 1
